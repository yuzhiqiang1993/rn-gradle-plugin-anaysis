...
abstract class GenerateCodegenArtifactsTask : Exec() {
    ...
    @get:Input
    abstract val codegenJavaPackageName: Property<String>
    ...
    @get:OutputDirectory
    val generatedJavaFiles: Provider<Directory> = generatedSrcDir.dir("java")

    @get:OutputDirectory
    val generatedJniFiles: Provider<Directory> = generatedSrcDir.dir("jni")

    override fun exec() {
        println("GenerateCodegenArtifactsTask exec")
        val (resolvedLibraryName, resolvedCodegenJavaPackageName) = resolveTaskParameters()
        println("resolvedLibraryName: $resolvedLibraryName, resolvedCodegenJavaPackageName: $resolvedCodegenJavaPackageName")
    ...
    internal fun resolveTaskParameters(): Pair<String, String> {
        ...
        val resolvedCodegenJavaPackageName =
            parsedPackageJson?.codegenConfig?.android?.javaPackageName
        ...
        commandLine(
            windowsAwareCommandLine(
                *nodeExecutableAndArgs.get().toTypedArray(),
                reactNativeDir.file("scripts/generate-specs-cli.js")
                    .get().asFile.cliPath(workingDir),
                "--platform",
                "android",
                "--schemaPath",
